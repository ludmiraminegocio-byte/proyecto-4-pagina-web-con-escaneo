...
// 2. Inicializar la IA de Gemini con tu clave secreta
const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);

// Función auxiliar para convertir la imagen a un formato que Gemini pueda leer
...
// 3. La ruta que recibe la foto y le pregunta a Gemini
app.post('/api/analizar-imagen', upload.single('imagen'), async (req, res) => {
    ...
    try {
        // Usamos el modelo 'flash' porque es rapidísimo para imágenes
        const modelo = genAI.getGenerativeModel({ model: "gemini-1.5-flash" });

        // Preparamos la imagen y las instrucciones para la IA
        ...
        // Borramos la imagen temporal del servidor para no saturar la memoria
        fs.unlinkSync(req.file.path);

        // Le enviamos el título a tu página web (admin.html)
        res.json({ 
